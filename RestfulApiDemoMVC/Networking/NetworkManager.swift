...
//    }
//    
//    // 專門用於不需要返回數據的請求（如 DELETE）
//    private func performRequestWithoutResponse(endpoint: String,
//                                            method: HTTPMethod,
//                                            body: Data? = nil,
//                                            completion: @escaping (Result<Void, Error>) -> Void) {
//        let urlString = "\(baseURL)\(endpoint)"
...
//        let endpoint = "/videos/\(videoId)?key=\(apiKey)"
//        
//        performRequest(endpoint: endpoint, method: .delete) { (result: Result<EmptyResponse, Error>) in
//            switch result {
//            case .success:
//                completion(.success(()))
//            case .failure(let error):
